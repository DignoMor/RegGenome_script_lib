...
class SplitFragmentsTest(unittest.TestCase):
    def setUp(self):
        ...
        self.__fragments_file = os.path.join(self.__test_dir, "fragments.tsv")
    ...
    def __download_example_data(self):
        ...
        with open(self.__fragments_file + ".gz", "wb") as f:
            f.write(response.content)
        
        fragment_df = pd.read_csv(self.__fragments_file + ".gz", 
                                  sep="\t", 
                                  compression="gzip",
                                  header=None,
                                  )
        fragment_df.to_csv(self.__fragments_file, 
                           sep="\t", 
                           index=False, 
                           header=False,
                           )
        os.remove(self.__fragments_file + ".gz")
        ...
        label_df.iloc[:, 1].replace("Opc", "OPC", inplace=True)
